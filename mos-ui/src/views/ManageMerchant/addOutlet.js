...
class addOutlet extends Component {
  ...
  handleBusinessTypeChange (selectedOptionType) {
    ...
    var self = this;
    self.setState({ selectedOptionType });
        var auth = localStorage.getItem('auth');
        var data = {};
        axios({
          method: 'get',
          url: API_ROOT+'api/outlets/findIfoutletExistsForBrand?merchantId='+self.props.match.params.merchantid+'&brandName='+self.state.selectedOptionType.label+'&outletName='+this.state.fields['locationId'],
          data: data,
  ...
  handleChange(field, e){
    ...
    if(field == 'locationId')
    {
        ...
        axios({
          method: 'get',
          url: API_ROOT+'api/outlets/findIfoutletExistsForBrand?merchantId='+self.props.match.params.merchantid+'&brandName='+self.state.selectedOptionType.label+'&outletName='+e.target.value,
          data: data,
          headers: {'Content-Type': 'multipart/form-data','Accept': 'application/hal+json', 'Authorization':auth}
...
makeDataforApi(text = ''){
    ...
    var data  = {
        ...
        brandId: this.state.selectedOptionType.value,
        brandName: this.state.selectedOptionType.label,
...
CREATE_API(data,text = ''){
    ...
    var that = this;
    that.setState({enablesave: false});
    return  axios({
        method: 'post',
        url: API_ROOT+'api/outlets',
        data: body,
        ...
        if(response.status == 200){
            if(text.length > 1){
               ...
               'sundayTo':moment(),
                ...
                if(that.props.match.params.msg == 'createoutlet')
                {
                  const { history } = that.props;
                  history.push('/managemerchant/'+that.props.match.params.merchantid+'/4/createoffer');
                }
                else
                {
                  const { history } = that.props;
                  history.push('/managemerchant/'+that.props.match.params.merchantid+'/3/outletcreated');  
                }
    ...
    return  axios({
        method: 'post',
        url: API_ROOT+'api/outlets/'+this.props.sendData.outletId,
        data: body,
        headers: {'Content-Type': 'application/json','Accept': 'application/hal+json', 'Authorization':auth}
        ...
        if(response.status == 200){
            if(text.length > 1){
               ...
               'sundayTo':moment(),
                ...
                that.props.change('TABLE_PAGE');
                const { history } = that.props;
                history.push('/managemerchant/'+that.props.match.params.merchantid+'/3/outletupdated');  
            }
...
validateField(){
    ...
    if(this.state.selectedOptionType == null || this.state.selectedOptionType.length ==0){
        formIsValid = false;
        errorBag['selectedOptionType'] = 'This field is required'
    }
    ...
    if(this.state.selected_COUNTRY.value != 'Singapore')
     if(!fields['roadName1']){
         formIsValid = false;
         errorBag['roadName1'] = 'This filed is required'
     }
    //  if(!fields['roadName2']){
    //     formIsValid = false;
    ...
    if(!fields['locality']){
        formIsValid = false;
        errorBag['locality'] = 'This filed is required'
    }
  ...
  loadBrands()
  {
        ...
        axios({
                method: 'get',
                url: API_ROOT+'api/brands/search/findByMerchantId/'+self.props.match.params.merchantid,
                data: body,
                headers: {'Content-Type': 'application/json','Accept': 'application/hal+json', 'Authorization':auth}
                ...
                .then(function (response) {
                  ...
                  self.setState({countryDropDown: [...self.state.countryDropDown, {value: obj.value,label: obj.label}]});
              });
              })
              .catch(function (error) {
                 //self.setState({roles:[]});
...
makeform(){
    ...
    fields['city'] = this.props.sendData.address.cityName;
    fields['streetNo'] = this.props.sendData.address.streetNo;
    fields['street'] = this.props.sendData.address.streetName;
    fields['buildingName'] = this.props.sendData.address.buildingName;
    ...
    fields['locality'] = this.props.sendData.address.locality;

    var bType = {"label":this.props.sendData.brandName,"value":this.props.sendData.brand.brandId};
            this.setState({selectedOptionType:bType});
  ...
  render() {
const { selected_COUNTRY, selectedOptionType } = this.state;
...
const valueBusinessType = selectedOptionType && selectedOptionType.value;
    ...
    return (
        <div className="animated fadeIn">
            ...
            <form style={{padding:20+'px'}} onSubmit= {this.createOutlet.bind(this)}>
              <FormGroup >
                    <Label htmlFor="locationId" className='font-14'  style={{fontWeight:600}}>Select Brand <span className="required">*</span></Label>
                    <Select
                      ...
                      />
                    {/* <FormText color="muted">Select Brand</FormText> */}
                    <FormText color='red'>{this.state.errorBag['selectedOptionType']}</FormText>
                ...
                <FormGroup >
                    ...
                    <Input type="text" id="locationId" value={this.state.fields['locationId']} onChange={this.handleChange.bind(this, "locationId")} name="locationId" placeholder="Location name"/>
                ...
                <FormGroup >
                    ...
                    <Row>
                      <Button color="save-button" onClick= {this.createOutlet.bind(this)}>{this.props.match.params.msg == 'crateoutlet' ? <span>Save & add offers</span> : <span>Save</span> }</Button>
                      &nbsp;&nbsp;
                      {this.props.sendData ? <span></span> : <span><Button color="primary" onClick={this.createAnother.bind(this)}> Add another outlet </Button></span> }
                    </Row>
